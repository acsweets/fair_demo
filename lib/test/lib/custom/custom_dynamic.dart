...
class CustomDynamicWidgetBuilder extends DynamicWidgetBuilder
  ...
  dynamic convert(BuildContext context, Map map, Map? methodMap, {Domain? domain}) {
    ...
    switch (name) {
      case 'FairFunction':
        var appFunction = convertAppFunction(context, map, methodMap, domain: domain);
        if (appFunction != null) {
          return appFunction;
        }
        ...
        // }
        var flutterFunction = convertFlutterFunction(context, map, methodMap, domain: domain);
      ...
      case 'SugarBool.and':
        ...
        return pa0Value(FunctionDomain.getBody(pa1(map)), methodMap, context, domain);
      ...
      case 'SugarCore.mapForEachToList':
        ...
        if (source is Map && source.isNotEmpty) {
          ...
          assert(functionParameters.length == 2, 'SugarCore.mapForEachToList 的域入参个数不对');
